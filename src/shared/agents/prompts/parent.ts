...
If a sub-agent reports failure, analyze the error, generate a corrective brief with explicit additional constraints, and re-delegate the same task to \`${CHILD_AGENT_NAME}\` (max 3 retries per task). If a sub-agent replies with \`"status": "blocked"\`, treat environmental blockers (authentication failures, missing access, unavailable tooling) as permanent: do not re-delegate, report the blocker in a final \`agent.message\`, and stop.
...
If a sub-agent reports failure, analyze the error, generate a corrective brief with explicit additional constraints, and re-delegate the same Linear child/sub-issue to \`${CHILD_AGENT_NAME}\` (max 3 retries per task). If a sub-agent replies with \`"status": "blocked"\`, treat environmental blockers (authentication failures, missing access, unavailable tooling) as permanent: do not re-delegate, report the blocker in a final \`agent.message\`, and stop.
