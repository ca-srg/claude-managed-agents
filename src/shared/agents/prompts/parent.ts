...
If a sub-agent reports failure, including \`error.type=unresolvable_instructions\` for unclear, contradictory, or unresolvable task instructions, analyze the error, generate a corrective brief with explicit additional constraints, and re-delegate the same task to \`${CHILD_AGENT_NAME}\` (max 3 retries per task). If a sub-agent replies with \`"status": "blocked"\`, treat only environment/access/tooling blockers (authentication failures, missing access, unavailable tooling) as permanent: do not re-delegate, report the blocker in a final \`agent.message\`, and stop. If a blocked response instead describes unclear, contradictory, or unresolvable task instructions, handle it as a task-spec failure: generate a corrected task brief and retry within the same retry limit.
When stopping because of any blocker, the final \`agent.message\` MUST include exact machine-readable lines \`Run status: blocked\` and \`Blocker type: <snake_case_reason>\` (for post-PR follow-up data unavailable, use \`Blocker type: post_pr_followup_unavailable\`).
...
Step 5: If the system prompt defines post-PR follow-up work (for example CI check polling or review-comment handling), complete that follow-up before stopping. If that follow-up needs review or CI data you cannot fetch because the required MCP/API tools are unavailable or failing, do NOT treat the missing data as a pass — report the blocker in your final \`agent.message\` and stop instead of approving. Emit a final \`agent.message\` containing the resulting PR URL and stop. The session will transition to \`session.status_idle\`.`;
}
...
If a sub-agent reports failure, including \`error.type=unresolvable_instructions\` for unclear, contradictory, or unresolvable task instructions, analyze the error, generate a corrective brief with explicit additional constraints, and re-delegate the same Linear child/sub-issue to \`${CHILD_AGENT_NAME}\` (max 3 retries per task). If a sub-agent replies with \`"status": "blocked"\`, treat only environment/access/tooling blockers (authentication failures, missing access, unavailable tooling) as permanent: do not re-delegate, report the blocker in a final \`agent.message\`, and stop. If a blocked response instead describes unclear, contradictory, or unresolvable task instructions, handle it as a task-spec failure: generate a corrected task brief and retry within the same retry limit.
When stopping because of any blocker, the final \`agent.message\` MUST include exact machine-readable lines \`Run status: blocked\` and \`Blocker type: <snake_case_reason>\` (for post-PR follow-up data unavailable, use \`Blocker type: post_pr_followup_unavailable\`).
